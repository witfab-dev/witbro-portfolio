...
const generateResponse = (userInput) => {
  ...
  // About / Who is
  if (input.match(/\b(about|who is|tell me about|introduce)\b/) && input.match(/\b(witness|fabrice|him|he)\b/)) {
  ...
  // Skills / Technologies
  if (input.match(/\b(skills|technologies|tech stack|programming|frameworks|tools)\b/)) {
  ...
  // Projects
  if (input.match(/\b(projects|built|created|developed|portfolio|work)\b/)) {
  ...
  // Education
  if (input.match(/\b(education|study|studied|school|college|university|degree|diploma|graduated)\b/)) {
  ...
  // Contact / Hire
  if (input.match(/\b(contact|hire|email|reach|connect|work with|freelance|job|opportunity)\b/)) {
  ...
  // Location
  if (input.match(/\b(location|based|where|city|country|rwanda|kigali)\b/)) {
  ...
  if (input.match(/\b(experience|background|journey|career)\b/)) {
  ...
  if (input.match(/\b(personality|work style|ethic|passionate|approach)\b/)) {
    return KNOWLEDGE_BASE.personality.slice(0, 2).join(" ") + ` ${KNOWLEDGE_BASE.personality[2]}`;
  }
  
  // Help / What can you do
  if (input.match(/\b(help|what can you do|capabilities|features)\b/)) {
...
export default function VoiceAssistant({ autoOpen = true, onClose }) {
  ...
  // ── Generate AI Response (No API needed!) ─────────────────────
  const getAIResponse = useCallback((userText) => {
  ...
  const process = useCallback(async (query) => {
    ...
    try {
      ...
      const msg = "Sorry, I encountered an issue. Please try again!";
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const welcomeMsg = "Hi there! 👋 I'm Witbri AI, your virtual assistant. Ask me anything about Witness Fabrice - his skills, projects, background, or how to hire him. What would you like to know?";
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] font-black tracking-tight text-white">Witbri AI</span>
