...
// ══════════════════════════════════════════════════════════════
// CSS Fallback Globe (when WebGL is unavailable)
// ══════════════════════════════════════════════════════════════
...
function GlobeBackground() {
  ...
  const {
    ...
    onInit: ({ scene }) => {
      ...
      // ── Helper: lat/lon to sphere position ──────────────
      ...
      globeGroup.add(
        new THREE.Points(dotGeo, new THREE.PointsMaterial({
      ...
      const cities = [
        ...
        { lat: -26.2,  lon: 28.04,  color: 0x10b981, r: 0.022 },
        { lat: 6.52,   lon: 3.38,   color: 0xf97316, r: 0.022 },
        { lat: 52.52,  lon: 13.40,  color: 0x3b82f6, r: 0.022 },
      ];
      ...
      // ✅ Animation is handled internally by the hook
      // The hook auto-renders every frame
      // We just update our objects here
      let elapsed = 0;
      const animate = () => {
        elapsed += 0.016;
      ...
      // Return the animation function for the hook to call
      return animate;
    },
  });
...
// ══════════════════════════════════════════════════════════════
// Globe Renderer (chooses WebGL or fallback)
// ══════════════════════════════════════════════════════════════
function GlobeRenderer() {
  const [webgl] = useState(() => isWebGLSupported());
  if (!webgl) return <GlobeFallback />;
  return (
    <LazyThreeJS componentId="contact-globe" fallback={<GlobeFallback />}>
      <GlobeBackground />
    </LazyThreeJS>
  );
}
...
export default function Contact() {
  ...
  const subjectOptions = [
    'Web / App Development', 'UI / UX Design', 'Technical Consulting',
    'Team Collaboration', 'Job Opportunity', 'Open Source', 'Other',
  ];
  ...
  const handleChange = (e) => {
    ...
    const keyMap = {
      from_name: 'name', reply_to: 'email', phone: 'phone',
  ...
  // ✅ FIXED: No more emailjs.init() here - already initialized at module level
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input name="from_name" type="text" required value={formData.name} onChange={handleChange} onFocus={() => setFocused('name')} onBlur={() => setFocused(null)} placeholder="Witness Fabrice" className={`${inputBase} ${focusCls('name')}`} />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[11px] font-bold uppercase tracking-widest text-white/45 ml-1">
                        {t('emailLabel', 'Email address')} <span className="text-orange-500">*</span>
                      </label>
                      <input name="reply_to" type="email" required value={formData.email} onChange={handleChange} onFocus={() => setFocused('email')} onBlur={() => setFocused(null)} placeholder="you@example.com" className={`${inputBase} ${focusCls('email')}`} />
                    </div>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input name="phone" type="tel" value={formData.phone} onChange={handleChange} onFocus={() => setFocused('phone')} onBlur={() => setFocused(null)} placeholder="+250 783 000 000" className={`${inputBase} ${focusCls('phone')}`} />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[11px] font-bold uppercase tracking-widest text-white/45 ml-1">Subject</label>
                      <select name="subject" value={formData.subject} onChange={handleChange} onFocus={() => setFocused('subject')} onBlur={() => setFocused(null)} className={`${inputBase} ${focusCls('subject')} appearance-none cursor-pointer`} style={{ backgroundColor: 'rgba(255,255,255,0.07)' }}>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <textarea name="message" required rows={6} value={formData.message} onChange={handleChange} onFocus={() => setFocused('message')} onBlur={() => setFocused(null)} placeholder="Tell me about your project…" className={`${inputBase} resize-none ${focusCls('message')}`} />
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mt-1">
                      <div className="flex flex-wrap gap-1.5">
                        {quickReplies.map(({ emoji, text }, i) => (
                          <button key={i} type="button" onClick={() => handleQuickReply(text)} className="text-[10px] px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-white/45 hover:text-white hover:border-orange-400/60 hover:bg-orange-500/10 transition-all">
                            {emoji} {text}
                          </button>
                        ))}
                      </div>
                      <p className={`text-[10px] tabular-nums shrink-0 ${charCount > 1000 ? 'text-orange-400' : 'text-white/35'}`}>{charCount} chars</p>
                    </div>
                  </div>
              ...
              {activeTab === 'collaborate' && (
                <motion.div key="collaborate-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }} className="space-y-5">
                  <div>
                    <h3 className="text-lg font-black text-white mb-1">Ways to work together</h3>
                    <p className="text-white/50 text-sm leading-relaxed">Whether you need a full product build, a quick consultation, or an extra hand on an existing project — I'm flexible.</p>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {collaborationItems.map(({ icon: Icon, title, desc }, i) => (
                      <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} className="group p-4 rounded-xl border border-white/10 bg-white/[0.04] hover:bg-orange-500/10 hover:border-orange-500/30 transition-all duration-300 cursor-pointer">
                        <div className="w-10 h-10 mb-3 rounded-xl bg-orange-500/15 border border-orange-500/20 flex items-center justify-center text-orange-400 group-hover:bg-orange-500 group-hover:text-white transition-all"><Icon size={18} /></div>
