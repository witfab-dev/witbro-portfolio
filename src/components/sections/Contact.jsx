  ...
  const { mountRef, isReady, error, startAnimationLoop, handleResize } = useThreeJS(
    'contact-globe',
    {
      ...
      onInit: ({ scene }) => {
        ...
        // Position globe right-of-center
...
// ─── Globe renderer (chooses Three.js or CSS fallback) ─────────
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto">
        ...
        >
          <p className="inline-flex items-center gap-2 text-[11px] font-semibold tracking-[0.22em] uppercase text-orange-500 mb-5 px-4 py-2 rounded-full bg-orange-500/10 backdrop-blur-sm border border-orange-500/20">
            <Globe size={12} /> {t('contact', 'Get in touch')}
          </p>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            {contactInfo.map((info, i) => {
              ...
              return (
                ...
                >
                  ...
                  {info.copyable && (
                    <button aria-label={`Copy ${info.label}`}
            ...
            {/* Socials */}
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.38 }}
            ...
            {/* Availability */}
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.46 }}
          ...
          {/* Right — form */}
          <motion.div
            initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, delay: 0.15 }}
            className="lg:col-span-2 bg-black/50 backdrop-blur-md border border-white/10 rounded-2xl p-6 sm:p-8"
          >
            {/* Tabs */}
            <div className="flex gap-2 mb-7">
              {[{ key: 'contact', label: '✉️  Contact' }, { key: 'collaborate', label: '🤝  Collaborate' }].map(({ key, label }) => (
            ...
            <AnimatePresence mode="wait">
              ...
              {/* Contact form */}
              {activeTab === 'contact' && (
                <motion.form key="contact-form" ref={formRef} onSubmit={handleSubmit}
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input name="from_name" type="text" required value={formData.name} onChange={handleChange}
                        onFocus={() => setFocused('name')} onBlur={() => setFocused(null)}
                        placeholder="Witness Fabrice" className={`${inputBase} ${focusCls('name')}`} />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[11px] font-bold uppercase tracking-widest text-white/45 ml-1">
                        {t('emailLabel', 'Email')} <span className="text-orange-500">*</span>
                      </label>
                      <input name="reply_to" type="email" required value={formData.email} onChange={handleChange}
                        onFocus={() => setFocused('email')} onBlur={() => setFocused(null)}
                        placeholder="you@example.com" className={`${inputBase} ${focusCls('email')}`} />
                    </div>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input name="phone" type="tel" value={formData.phone} onChange={handleChange}
                        ...
                        placeholder="+250 783 000 000" className={`${inputBase} ${focusCls('phone')}`} />
                    </div>
                    <div className="flex flex-col gap-1.5">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 mt-1">
                      <div className="flex flex-wrap gap-1.5">
                        {quickReplies.map(({ emoji, text }, i) => (
                          <button key={i} type="button" onClick={() => handleQuickReply(text)}
              ...
              {activeTab === 'collaborate' && (
                <motion.div key="collaborate-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }} className="space-y-5">
                  ...
                  <div className="pt-2 flex flex-col sm:flex-row gap-3">
                    <a href="mailto:witnessfabrice@gmail.com?subject=Collaboration Inquiry"
                  ...
                  <p className="text-center text-[11px] text-white/35 pt-1">
                    ...
                    — available <span className="text-white/60 font-semibold">Mon–Fri 08:00–18:00</span>
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
